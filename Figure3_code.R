...
plot_LRL<- ggplot(dat_LRD, aes(x = Genotype, y = LRL, color = Genotype, show.legend = TRUE)) + 
...
plot_LRD<- ggplot(dat_LRD, aes(x = Genotype, y = PRL, show.legend = FALSE)) + 
...
plot_LRL<- ggplot(dat_LR_14b, aes(x = Genotype, y = LRL, show.legend = TRUE, color=Genotype), alpha=0.5) + 
  geom_boxplot(lwd=.75, outlier.size = 0, alpha = (0.5)) +
  geom_jitter(size =.5, width = 0.1,alpha=0.5) +
  coord_cartesian(ylim = c(0,20))+
  ylab('Lateral root length (mm)') +
